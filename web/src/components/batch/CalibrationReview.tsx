...
function ItemCard({
  ...
  return (
    ...
    >
      ...
      {/* Diff rows — only show when not rejected */}
      {!isRejected && item.status !== 'failed' && item.status !== 'skipped' && (
        <div className="space-y-2.5">
          <DiffRow label="Başlık" before="" after="" />
          <DiffRow label="Meta Başlık" before="" after="" />
        </div>
      )}
